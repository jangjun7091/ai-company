...
- Founder-interview discipline for the Idea Brief and Product Spec in `company/workflows/idea-to-release.md`: the two artifacts the agent builds toward are now elicited through a bounded interview, not drafted unilaterally. Reuses the `bootstrap-interview.md` interviewer stance with three stage-specific additions — ground ambition in feasibility, turn every "good" into an observable/machine-checkable acceptance criterion, and distill rather than transcribe (a few focused rounds, stop when decision-grade). Wired from `AGENTS.md` Feature work and pointer comments in both templates. (#20)
