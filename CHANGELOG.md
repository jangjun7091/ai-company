...
- First-class agent collaboration guidance for delegated build work: handoffs now stay current by referencing canonical sources and exposing provisional judgments for challenge; parallel runs scale only across disjoint ownership while accounting for serialized review and integration; unattended work carries artifact-based completion signals, explicit bounds, durable checkpoints, and recoverable interruption state; and orchestrator verification attributes each diff, controls for load, and separates observed failures from proposed causes. A new wired `templates/worker-handoff.md` carries these per-run details while leaving the durable task record unchanged. (#74)

- A named **Build–Verify autonomy span** in `company/workflows/idea-to-release.md`: after the Product Spec and any Design Brief are ratified, agents proceed through Build and Verify without founder interaction by default, interrupting only for the escalation policy or existing Safety-and-permissions gates and batching non-blocking questions to phase boundaries; Release and every human gate remain unchanged. A new per-mission Progress Journal (`templates/progress-journal.md`) records decision-grade checkpoints — completed tasks, decisions and why, surprises, and next actions — so compacted or resumed sessions reconstruct the build from files rather than chat; the journal is wired into `doctor.py`'s REQUIRED templates and its fixture test. The workflow also treats orchestrator context as a coherence resource, delegating token-heavy, well-specified work by default while the main session retains plan, state, and verification. (#69)

- An instruction-pruning discipline for the operating contract: every rule must trace to an observed failure or live decision, guidance that no longer changes behavior is removed in favor of fewer, stronger constraints, and substantive removals are recorded in ADRs. The weekly operating review now audits `AGENTS.md`, workflows, and vendor adapters for stale, duplicated, or over-constraining guidance so operating documents continue to serve model judgment instead of accreting into a cage. (#70)

- A **design-discovery seed** for missions with a user-facing surface: a "design pass" in the Specify layer of `company/workflows/idea-to-release.md` produces a Design Brief (`templates/design-brief.md`) alongside the Product Spec; backend and internal missions skip it, and the Idea Brief carries only a thin `Experience intent` hook so it keeps its intent altitude. The pass reuses the `bootstrap-interview.md` stance with design-specific shifts: the altitude line moves (a designer owns look and feel, layout, and visual language; the agent probes intent, proposes options, and implements — the boundary is recorded per mission, not assumed), "good" splits into verifiable proxies (accessibility, responsiveness, flow completeness, intent alignment — each promoted to a Spec acceptance criterion) versus aesthetic ratification (human-judged, iterated on references and anti-references), and shared-shell uniformity is treated as a hypothesis until a second variant confirms it. Execution maps onto existing environment design skills through a new `.claude/skills/design-discovery` adapter — the neutral layer stays vendor-free (`MAINTAINING.md` invariant 2). `design-brief.md` joins `doctor.py`'s REQUIRED templates (wired templates are required), locked by the fixture test. Deliberately thin: a seed to harden from the first real UX mission, not a design-artifact suite. (#67)
