...
Python 3.10+ 표준 라이브러리만 사용합니다 — 설치할 의존성이 없습니다. `python scripts/check.py` 한 명령으로 두 가드레일(계약 검사와 테스트 스위트)을 모두 실행하며, 둘 중 하나라도 실패하면 0이 아닌 코드로 종료합니다. `Makefile`(`make doctor`, `make test`)은 선택적 편의 기능이며 Windows에 기본 없는 GNU Make가 필요합니다.
...
- `scripts/` — 결정론적 검사 (`doctor.py`, `session_start.py`, `check.py`).
